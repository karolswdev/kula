...
**Version:** 2.0
**Status:** Grid Engine Evolution

## Introduction

This document outlines the software requirements for the **Kula World Clone**, a browser-based 3D puzzle-platformer inspired by the PlayStation classic, now evolving into a comprehensive **Grid-Based Game Creation Engine**. It serves as the single source of truth for what the system must do, the constraints under which it must operate, and the rules governing its development and deployment.

The system is being architected around a **Universal Grid System** that will enable rapid content creation, predictable physics, and community-driven level design.
...
| <a name="PROD-013"></a>**PROD-013** | **Grid System:** Universal 3D Grid | The world **MUST** be defined using integer grid coordinates `[x, y, z]` with a configurable `gridUnitSize` (default: 4) for world-space translation. All static geometry **MUST** align to this grid. | This provides speed in level design, predictable gameplay mechanics, and enables algorithmic optimization of physics bodies. |
| <a name="PROD-014"></a>**PROD-014** | **Asset Registry:** Visual-Logic Decoupling | The system **MUST** implement a centralized Block & Asset Registry that maps logical block types to visual models (e.g., `Rock Medium.glb`, `Cube Bricks.glb`, `Stone Platform.glb`), physics primitives, and grid footprints. | This enables theming, asset normalization, and performance optimization through simplified collision geometry. |
| <a name="PROD-015"></a>**PROD-015** | **Declarative Behaviors:** Smart Blocks | Blocks **MUST** support declarative behavior attachments defined in level data (e.g., elevators, timed traps, moving platforms). A spike trap hazard **MUST** use the `Hazard Spike Trap.glb` model with appropriate trigger behaviors. | This empowers level designers to create dynamic puzzles without coding, increasing content variety and replayability. |
